...
FDSal2 <- read.csv('DKSalaries (101) (1).csv', stringsAsFactors=FALSE)
...
plyr_sd <- read.csv('snapsthruwk15.csv')
...
rztar <- read.csv('RZtargetsthruwk15.csv')
...
rzrush <- read.csv('RZrushingthruwk15 (1).csv')
...
nfjoin <- read.csv('targetsthruwk15.csv', stringsAsFactors=FALSE)
...
nfjoin2 <- read.csv('touchesthruwk15.csv', stringsAsFactors=FALSE)
...
joinedxFD <- left_join(joinedzFD, nfjoin, by = "Name")
joinedxFD$Name <- as.character(joinedxFD$Name)
jz <- str_split_fixed(joinedxFD$Name, " ", 2)
jz
nm <- substr(jz[,1], 0, 1)
joinedxFD$Name <- paste(nm, jz[,2])
uninames <- data.frame(table(joinedxFD$Name))
uninames
write.csv(uninames, file='nfluninames.csv', row.names=FALSE)
write.csv(joinedxFD, file='joinedxFD.csv', row.names=FALSE)
